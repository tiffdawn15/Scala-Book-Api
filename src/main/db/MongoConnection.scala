...
object MongoDbConnection {
  ...
  def from(
    ...
    val connection = uri.split("://") match {
      // Local
      case Array("mongodb", host) =>
        MongoConnection.classic(host, 27017, credential)
      ...
      case _ =>
        throw IllegalArgumentException(
          "MongoDb uri must be mongodb://{host} or mongodb+srv://{host}, with no {port}"
        )
    }
  ...
  object BookDBConn {
    ...
    def from(env: Map[String, String] = Map.empty): BookDBConn = {
      val connection =
        MongoDbConnection.from(env, URI_KEY, DB_KEY, USERNAME_KEY, PASSWORD_KEY)
